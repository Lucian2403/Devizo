...
function buildSystemPrompt(catalogLanguage: string): string {
  ...
  return [
    ...
    `- object: exactly one of [${objects}] or null. The thing acted upon, NOT the surface. tapet/oboi/обои → wallpaper; faianță/gresie/плитка → tiles; gips-carton → drywall; șapă/стяжка → screed; glet/șpacluire → putty; vopsea → paint; laminat/parchet → flooring; priză/розетка → socket; plașă/fibră de sticlă → mesh; hidroizolație/hidroizolare/гидроизоляция → waterproofing (NOT tiles/mesh); WC/lavoar/chiuvetă/duș/cadă → sanitaryware; țeavă/conductă/canalizare → pipe.`,
    ...
    "- DO NOT MERGE — general invariant: keep two requirements SEPARATE whenever ANY of these differ: surface, spatial region, quantity source, geometry formula, specification, object, unit, or required missing information. Never merge just because they share the same catalog item, action, or sentence. Only merge when it is the SAME operation over the SAME surface/object/specification and quantities can be safely summed. So: cold water (apă rece), hot water (apă caldă), sewer (canalizare) → 3 items even under one action; floor waterproofing vs wall perimeter waterproofing → 2 (different geometry); prize vs întrerupătoare → 2 (different object); pereți vs tavan → 2 (different surface). Mount type (WC suspendat/încastrat vs pe pardoseală) is a hard specification — keep it in specifications and description.",
    "",
