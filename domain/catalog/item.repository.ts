...
export interface CatalogItemRepository {
  ...
  // Search-as-you-type and AI lexical retrieval share this query. Currency is
  // optional because the UI picker needs to show incompatible rows, while AI
  // matching should only retrieve rows usable by the quote being edited.
  searchActive(
    organizationId: OrganizationId,
    term: string,
    limit: number,
  ...
  // Exact cosine vector search. The optional currency filter is applied before
  // ranking and LIMIT so compatible rows cannot be crowded out by another
  // currency.
  semanticSearch(
    organizationId: OrganizationId,
